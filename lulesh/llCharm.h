...
//    void ResumeFromSync();
  public:
    ...
    void receiveNodalMass(int dir, int width, int height,
                       Real_t ghostData[]);
	void receiveNodeGhosts(int iter, int dir, int width, int height, Real_t ghostData[]);
	void receiveElemGhosts(int iter, int dir, int width, int height, Real_t ghostData[]);
    void averageTime();
    void printEnergy();
};
