 ...
 *
 *   NOTE: The P2P comment lines remove the contributions and make lulesh point-to-point. This is a 
 *   temporary change to test the new message-logging protocol.
 */
...
#include "pup_stl.h"
#include "llCharm.h"
double ckpt_period_time = 1800.6;
//double ckpt_period_time = 19.37;
int ckpt = 3600;
float LB_PERIOD=90;
float avg_it;
//
...
void writeFileEnergyFaultDouble1(char *fname,double val)
{
...
double readFileEnergyFaultDouble1(char *fname)
{
        double val;
...
Main::Main(CkArgMsg *msg) {
  ...
  if ((msg->argc != 6) && (msg->argc != 10)) {
    CkPrintf("Usage: Takes exactly 4 or 8 command line arguments\n"
             "  %s [elemDim] [blockDim] [iterations] [ckptFreq]\n"
             "  %s [elemDimX] [elemDimY] [elemDimZ] [blockDimX] [blockDimY] [blockDimZ] [iterations] [ckptFreq]\n",
    ...
    maxIterations = atoi(msg->argv[3]);
    cpFrequency = atoi(msg->argv[4]);
		ckpt_period_time = atof(msg->argv[5]);
  }
  else {
    ...
    maxIterations = atoi(msg->argv[7]);
    cpFrequency = atoi(msg->argv[8]);
		ckpt_period_time = atof(msg->argv[9]);
  }
	ckpt = ckpt_period_time/0.504;
  ckpt_period = ckpt;
...
void Main::updateLastT()
{
	lastT = CmiWallTimer();
	CkPrintf("TTTTTTTTT Updating lastT:%f\n",lastT);
}

void Domain:: ResumeFromSync()
{
...
void Domain::doSync(){ AtSync();}

void Main::report(CkReductionMsg *m){
	iteration = ((int *)m->getData())[0];
	//lbTime -= avg_it;
	lbTime -= CmiWallTimer()-lastT;
	lbItrs++;
	//CkPrintf("->> %d %d\n",iteration, maxIterations);
	if(iteration%2==0) CkPrintf("Starting iteration %d t:%f lbTime:%f avg_it:%f time/itr:%.4f\n",iteration,CmiWallTimer()-startT,lbTime,avg_it,(CmiWallTimer()-startT)/iteration);
	if(iteration < maxIterations && (CmiWallTimer()-startT)<300)
	{
    //if(iteration > 0 && iteration % lbFrequency == lbFrequency - 1)
//		else
		{
			if(iteration != 0 && lastckpt + ckpt_period <= iteration){
				writeFileEnergyFaultDouble1("ckptperiod.txt",ckpt_period_time);
//        CkCallback cb (CkIndex_Domain::begin_iteration(), domains);
				CkCallback cb(CkIndex_Main::afterCkpt(),thisProxy);
        CkStartMemCheckpoint(cb);
        double ckpt_p = CmiWallTimer()-readFileEnergyFaultDouble1("ckptlast.txt");
        avg_it = ckpt_p / ckpt_period;
        int ckpt_period_t = (int) ckpt_period_time / avg_it;
        if(ckpt_p>4.0)
        {
          lastckpt = iteration;
          ckpt_period = roundUp(ckpt_period_t,2);
					if(( lastckpt + ckpt_period) % lbFrequency == lbFrequency - 1) ckpt_period=ckpt_period+2;
          CkPrintf("++++++++++++++++ INFO ckpt_period_last:%f avg_it:%.3f lastckpt:%d ckpt_period:%d nectckpt:%d\n",ckpt_p,avg_it,lastckpt,ckpt_period,lastckpt + ckpt_period);
        }
        else 
				{
    ...
    else if(lbTime<0 )
    {
        ...
        double setKillTime = readFileEnergyFaultDouble1("setKillTime.txt");
        double killTime = readFileEnergyFaultDouble1("killTime.txt");
        double rightNow = CmiWallTimer();
        if(setKillTime+killTime < rightNow + 2)
        {
          CkPrintf("!!!!!!!!!!!!! Avoiding LDB !!!!!!!!!!! there is a restart scheduled\n");
          domains.begin_iteration();
        }
        else
				{
          ...
          domains.doSync();
					CkPrintf("XXXXXXX ITR:%d avg_it:%.3f lbItrs:%d lastckpt:%d ckpt_period:%d nextckpt:%d\n",
            iteration,avg_it,lbItrs,lastckpt,ckpt_period,roundUp(lastckpt + ckpt_period,2));
				}
        lbTime = LB_PERIOD;
				lbItrs = 0;
    }
...
Main::averageTimeCheckin(CkReductionMsg *msg) {
  ...
  CkPrintf("Lulesh (Charm++) Completed\n"
           "  Simulation Time: %.12e (wallclock, in seconds)\n"
           "  Total Iterations: %d\n",
           iterTimers[1], totalIterations);
...
void Domain::begin_iteration()
{
		//if(thisIndex.x == 0 && thisIndex.y == 0 && thisIndex.z == 0 && iterations%2==0)
    //CkPrintf("Starting iteration %d t:%f numNodeGhosts:%d\n",iterations,CmiWallTimer(),numNodeGhosts);
...
void Main::initCheckpoint(){
  //CkCallback cb(CkIndex_Domain::recvCheckPointDone(), domains);
...
Domain::Domain() {
  //__sdag_init();
  usesAtSync = CmiTrue;
	double writeckpt = ckpt;
	if (thisIndex.x==0 && thisIndex.y==0 && thisIndex.z==0)
		writeFileEnergyFaultDouble1("ckpt_period_itr.txt",writeckpt);
  iterations = 0;
  ghostElemCount = 0;
  ghostNodeCount = 0;
...
#endif
//	if(thisIndex.x == 0 && thisIndex.y == 0 && thisIndex.z == 0)
//		CkPrintf("Starting iteration %d t:%f\n",iterations,CmiWallTimer());
  // Computation
...
Domain::resumeElemIteration() {
  ...
  //P2P contribute(sizeof(Real_t), &domain->deltaTime, min_real);
#else
  //P2P contribute(sizeof(Real_t), NULL, CkReduction::nop);
#endif
}
...
Domain::sendNodeGhosts() {
  ...
  if (thisIndex.x != chareDimX-1 && thisIndex.y != chareDimY-1 && thisIndex.z != chareDimZ-1) {
    ...
    thisProxy(thisIndex.x+1,thisIndex.y+1,thisIndex.z+1)
      .receiveNodeGhosts(iterations,CORNER_FRU,1,1,cornerGhost);
  }
  if (thisIndex.x != chareDimX-1 && thisIndex.y != chareDimY-1 && thisIndex.z != 0) {
    ...
    thisProxy(thisIndex.x+1,thisIndex.y+1,thisIndex.z-1)
      .receiveNodeGhosts(iterations,CORNER_FRD,1,1,cornerGhost);
  }
  if (thisIndex.x != chareDimX-1 && thisIndex.y != 0 && thisIndex.z != chareDimZ-1) {
    ...
    thisProxy(thisIndex.x+1,thisIndex.y-1,thisIndex.z+1)
      .receiveNodeGhosts(iterations,CORNER_FLU,1,1,cornerGhost);
  }
  if (thisIndex.x != chareDimX-1 && thisIndex.y != 0 && thisIndex.z != 0) {
    ...
    thisProxy(thisIndex.x+1,thisIndex.y-1,thisIndex.z-1)
      .receiveNodeGhosts(iterations,CORNER_FLD,1,1,cornerGhost);
  }
  if (thisIndex.x != 0 && thisIndex.y != chareDimY-1 && thisIndex.z != chareDimZ-1) {
    ...
    thisProxy(thisIndex.x-1,thisIndex.y+1,thisIndex.z+1)
      .receiveNodeGhosts(iterations,CORNER_BRU,1,1,cornerGhost);
  }
  if (thisIndex.x != 0 && thisIndex.y != chareDimY-1 && thisIndex.z != 0) {
    ...
    thisProxy(thisIndex.x-1,thisIndex.y+1,thisIndex.z-1)
      .receiveNodeGhosts(iterations,CORNER_BRD,1,1,cornerGhost);
  }
  if (thisIndex.x != 0 && thisIndex.y != 0 && thisIndex.z != chareDimZ-1) {
    ...
    thisProxy(thisIndex.x-1,thisIndex.y-1,thisIndex.z+1)
      .receiveNodeGhosts(iterations,CORNER_BLU,1,1,cornerGhost);
  }
  if (thisIndex.x != 0 && thisIndex.y != 0 && thisIndex.z != 0) {
    ...
    thisProxy(thisIndex.x-1,thisIndex.y-1,thisIndex.z-1)
      .receiveNodeGhosts(iterations,CORNER_BLD,1,1,cornerGhost);
  }
  // z edges
  if (thisIndex.x != chareDimX-1 && thisIndex.y != chareDimY-1) {
    ...
    thisProxy(thisIndex.x+1, thisIndex.y+1, thisIndex.z)
      .receiveNodeGhosts(iterations,EDGE_PXPY,1,ghostDimZ,zEdge);
  }
  if (thisIndex.x != chareDimX-1 && thisIndex.y != 0) {
    ...
    thisProxy(thisIndex.x+1, thisIndex.y-1, thisIndex.z)
      .receiveNodeGhosts(iterations,EDGE_PXMY,1,ghostDimZ,zEdge);
  }
  if (thisIndex.x != 0 && thisIndex.y != chareDimY-1) {
    ...
    thisProxy(thisIndex.x-1, thisIndex.y+1, thisIndex.z)
      .receiveNodeGhosts(iterations,EDGE_MXPY,1,ghostDimZ,zEdge);
  }
  if (thisIndex.x != 0 && thisIndex.y != 0) {
    ...
    thisProxy(thisIndex.x-1, thisIndex.y-1, thisIndex.z)
      .receiveNodeGhosts(iterations,EDGE_MXMY,1,ghostDimZ,zEdge);
  }
  // y edges
  if (thisIndex.x != chareDimX-1 && thisIndex.z != chareDimZ-1) {
    ...
    thisProxy(thisIndex.x+1, thisIndex.y, thisIndex.z+1)
      .receiveNodeGhosts(iterations,EDGE_PXPZ,1,ghostDimY,yEdge);
  }
  if (thisIndex.x != chareDimX-1 && thisIndex.z != 0) {
    ...
    thisProxy(thisIndex.x+1, thisIndex.y, thisIndex.z-1)
      .receiveNodeGhosts(iterations,EDGE_PXMZ,1,ghostDimY,yEdge);
  }
  if (thisIndex.x != 0 && thisIndex.z != chareDimZ-1) {
    ...
    thisProxy(thisIndex.x-1, thisIndex.y, thisIndex.z+1)
      .receiveNodeGhosts(iterations,EDGE_MXPZ,1,ghostDimY,yEdge);
  }
  if (thisIndex.x != 0 && thisIndex.z != 0) {
    ...
    thisProxy(thisIndex.x-1, thisIndex.y, thisIndex.z-1)
      .receiveNodeGhosts(iterations,EDGE_MXMZ,1,ghostDimY,yEdge);
  }
  // x edges
  if (thisIndex.y != chareDimY-1 && thisIndex.z != chareDimZ-1) {
    ...
    thisProxy(thisIndex.x, thisIndex.y+1, thisIndex.z+1)
      .receiveNodeGhosts(iterations,EDGE_PYPZ,1,ghostDimX,xEdge);
  }
  if (thisIndex.y != chareDimY-1 && thisIndex.z != 0) { 
    ...
    thisProxy(thisIndex.x, thisIndex.y+1, thisIndex.z-1)
      .receiveNodeGhosts(iterations,EDGE_PYMZ,1,ghostDimX,xEdge);
  }
  if (thisIndex.y != 0 && thisIndex.z != chareDimZ-1) {
    ...
    thisProxy(thisIndex.x, thisIndex.y-1, thisIndex.z+1)
      .receiveNodeGhosts(iterations,EDGE_MYPZ,1,ghostDimX,xEdge);
  }
  if (thisIndex.y != 0 && thisIndex.z != 0) {
    ...
    thisProxy(thisIndex.x, thisIndex.y-1, thisIndex.z-1)
      .receiveNodeGhosts(iterations,EDGE_MYMZ,1,ghostDimX,xEdge);
  }
  ...
  if (thisIndex.x != chareDimX-1) {
    thisProxy(thisIndex.x+1,thisIndex.y,thisIndex.z)
      .receiveNodeGhosts(iterations,DIR_FRONT,ghostDimY,ghostDimZ,frontGhost);
  }
  // back  (-x)
  if (thisIndex.x != 0) {
    thisProxy(thisIndex.x-1,thisIndex.y,thisIndex.z)
      .receiveNodeGhosts(iterations,DIR_BACK,ghostDimY,ghostDimZ,backGhost);
  }
  // right (+y)
  if (thisIndex.y != chareDimY-1) {
    thisProxy(thisIndex.x,thisIndex.y+1,thisIndex.z)
      .receiveNodeGhosts(iterations,DIR_RIGHT,ghostDimX,ghostDimZ,rightGhost);
  }
  // left  (-y)
  if (thisIndex.y != 0) {
    thisProxy(thisIndex.x,thisIndex.y-1,thisIndex.z)
      .receiveNodeGhosts(iterations,DIR_LEFT,ghostDimX,ghostDimZ,leftGhost);
  }
  // up    (+z)
  if (thisIndex.z != chareDimZ-1) {
    thisProxy(thisIndex.x,thisIndex.y,thisIndex.z+1)
      .receiveNodeGhosts(iterations,DIR_UP,ghostDimX,ghostDimY,upGhost);
  }
  // down  (-z)
  if (thisIndex.z != 0) {
    thisProxy(thisIndex.x,thisIndex.y,thisIndex.z-1)
      .receiveNodeGhosts(iterations,DIR_DOWN,ghostDimX,ghostDimY,downGhost);
  }
...
// CODING
void Domain::receiveNodeGhosts(int iter, int dir, int width, int height, Real_t ghostData[]){
	processNodeGhosts(dir, width, height, ghostData);
	ghostNodeCount++;
	//if (thisIndex.x==0 && thisIndex.y==0 && thisIndex.z==0)
	//	CkPrintf("Recv -------->%d %d\n",ghostNodeCount,iterations);
	if(ghostNodeCount == numNodeGhosts){
		ghostNodeCount = 0;
          updateForceGhosts();
          resumeNodeIteration();
          sendElemGhosts();
// SHOULD THERE BE A SYNC HERE???? CHECK WITH ORIGINAL CODE
          //resumeElemIteration();
          //domain->deltaTime = DT_FIXED;
          //domain->totalTime += domain->deltaTime;
          //iterations++;

		//thisProxy[thisIndex].run1();
...
Domain::sendElemGhosts() {
  ...
  if (thisIndex.x != chareDimX-1) {
    thisProxy(thisIndex.x+1,thisIndex.y,thisIndex.z)
      .receiveElemGhosts(iterations,DIR_FRONT,blockDimY,blockDimZ,frontGhost);
  }
  // back  (-x)
  if (thisIndex.x != 0) {
    thisProxy(thisIndex.x-1,thisIndex.y,thisIndex.z)
      .receiveElemGhosts(iterations,DIR_BACK,blockDimY,blockDimZ,backGhost);
  }
  // right (+y)
  if (thisIndex.y != chareDimY-1) {
    thisProxy(thisIndex.x,thisIndex.y+1,thisIndex.z)
      .receiveElemGhosts(iterations,DIR_RIGHT,blockDimX,blockDimZ,rightGhost);
  }
  // left  (-y)
  if (thisIndex.y != 0) {
    thisProxy(thisIndex.x,thisIndex.y-1,thisIndex.z)
      .receiveElemGhosts(iterations,DIR_LEFT,blockDimX,blockDimZ,leftGhost);
  }
  // up    (+z)
  if (thisIndex.z != chareDimZ-1) {
    thisProxy(thisIndex.x,thisIndex.y,thisIndex.z+1)
      .receiveElemGhosts(iterations,DIR_UP,blockDimX,blockDimY,upGhost);
  }
  // down  (-z)
  if (thisIndex.z != 0) {
    thisProxy(thisIndex.x,thisIndex.y,thisIndex.z-1)
      .receiveElemGhosts(iterations,DIR_DOWN,blockDimX,blockDimY,downGhost);
  }
...
void Domain::receiveElemGhosts(int iter, int dir, int width, int height, Real_t ghostData[]){
	processElemGhosts(dir, width, height, ghostData);
	ghostElemCount++;
	if(ghostElemCount == numElemGhosts){
		ghostElemCount = 0;
