...
c---------------------------------------------------------------------
     ...
     >                 u1(ntdivnp), 
     >                 u2(ntdivnp)
      double precision twiddle(ntdivnp),lastItr,startT,startB
c---------------------------------------------------------------------
...
c---------------------------------------------------------------------
     ...
     >                                dims(3,1))
      ...
      do iter = 1, niter
         ...
         if(me.eq.0.and.mod(iter,10).eq.0) then 
            print *,'Itr =',iter,'Itr time=',MPI_Wtime()-lastItr,
     >       ' Tot time=', MPI_Wtime()-startT,' outer loop=',big
            lastItr = MPI_Wtime()           
         end if
c         if(iter.eq.955) then
c           call ftracebegin()
c         end if
c         if(iter.eq.965) then
c           call ftraceend()
c         end if

         if(iter.eq.598.and.big.eq.1) then
           call ftracebegin()
         end if
         if(iter.eq.602.and.big.eq.1) then
           call ftraceend()
         end if         
...
c      if (me .eq. 0) then
c            write (*, 30) i, allchk
c 30         format (' T =',I5,5X,'Checksum =',1P2D22.12)
c      endif
