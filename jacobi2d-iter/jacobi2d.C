...
//    void report(int row, int col) {
			void report(CkReductionMsg *m) {
				iterations=((int *)m->getData())[0];
        recieve_count++;
        double totaltime = CkWallTimer() - startTime;
//        if (num_chares == recieve_count) {
					if (1 == recieve_count) {
            if (iterations == total_iterations || CmiWallTimer()-stTime>=300) {
//                CkPrintf("Completed %d iterations; last iteration time: %.6lf\n", iterations, totaltime);
                ...
                if(iterations%2==0) CkPrintf("starting new iteration; iteration %d time: %.6lf time/itr::%.6f\n", iterations, CmiWallTimer()-stTime,(CmiWallTimer()-stTime)/iterations);
                recieve_count=0;
                iterations++;
                // Call begin_iteration on all worker chares in array
                startTime = CkWallTimer();
//               array.begin_iteration();
      if(iterations != 0 && iterations % 30 == 3000){
        CkCallback cb (CkIndex_Jacobi::begin_iteration(), array);
